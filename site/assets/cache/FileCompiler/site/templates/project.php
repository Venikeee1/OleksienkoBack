<?php include(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/templates/_header.php',array('includes'=>true,'namespace'=>true,'modules'=>true,'skipIfNamespace'=>true))); // include header markup ?>
<?php 
    if ($page->prev instanceof \ProcessWire\NullPage) {
        $prevPage = null;
    } else {
        $prevPage = $page->prev;
    }
    ...
    if ($page->next instanceof \ProcessWire\NullPage) {
        $nextPage = null;
    } else {
        $nextPage = $page->next;
    }
...
<div class="inner-page" >
    ...
    <section class="inner-page__slides">
        <?php if($page->slides) { ?>
            ...
            <?php } ?>
        <?php } ?>
        <div class="inner-page__slide" data-section="footer"><?php include(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/templates/contacts.inc',array('includes'=>true,'namespace'=>true,'modules'=>true,'skipIfNamespace'=>true))); ?></div>
    </section>
